...
import { Search, Check, Flame, ChevronDown, ChevronUp, Palette } from 'lucide-react';
import { useAppStore } from '../../store/useAppStore';
import { styleData, STYLE_CATEGORIES } from '../../data/styles';
import { scrollToSection } from '../../hooks/useAutoScroll';

const POPULAR_STYLES = ['오리지널 캐릭터', '짱구', '포켓몬', '지브리', '원피스', '귀멸의 칼날'];
const ORIGINAL_STYLE_KEY = '오리지널 캐릭터';
...
export default function StyleSelector() {
  const {
    ...
    setStyle,
    setCustomStyleInput,
    setOriginalStylePrompt,
  } = useAppStore();
  const [search, setSearch] = useState('');
  const [activeCategory, setActiveCategory] = useState<string | null>(null);
  const [collapsed, setCollapsed] = useState(true);
  const isOriginalSelected = selectedStyle?.name === ORIGINAL_STYLE_KEY;
  const collapsedLabel = useMemo(() => {
    if (selectedStyle?.name === ORIGINAL_STYLE_KEY) {
      return originalStylePrompt.trim()
        ? `${ORIGINAL_STYLE_KEY} · ${originalStylePrompt.trim()}`
        : ORIGINAL_STYLE_KEY;
    }
    return selectedStyle?.name || customStyleInput.trim() || '';
  }, [selectedStyle, customStyleInput, originalStylePrompt]);
  ...
  const handleSelect = (style: typeof styleData[0]) => {
    setStyle(style);
    setCollapsed(style.name !== ORIGINAL_STYLE_KEY);
      ...
      >
        <span className="flex items-center gap-2.5">
          ...
          {collapsed && collapsedLabel && (
            <span className="text-xs text-muted font-normal ml-1 truncate max-w-[180px]">
              {collapsedLabel}
            </span>
          )}
        </span>
        ...
        <div className="mt-3">
          ...
          {isOriginalSelected && (
            <div className="mt-3 space-y-1.5">
              <label className="text-xs font-semibold text-primary">오리지널 스타일 설명</label>
              <input
                type="text"
                value={originalStylePrompt}
                onChange={(e) => setOriginalStylePrompt(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && originalStylePrompt.trim()) setCollapsed(true);
                }}
                onBlur={() => {
                  if (originalStylePrompt.trim()) setCollapsed(true);
                }}
                placeholder="예: 열혈 모험 판타지 st (st=분위기만 유사, 캐릭터는 오리지널)"
                className="w-full px-4 py-2 text-sm border border-primary/40 rounded-xl bg-white shadow-[inset_0_1px_2px_rgba(0,0,0,0.06)] focus:outline-none focus:border-primary dark:bg-card"
                aria-label="오리지널 스타일 설명 입력"
              />
            </div>
          )}

          <div className="mt-3 space-y-1.5">
            <label className="text-xs font-semibold text-muted">직접 스타일 입력 (선택)</label>
            <input
              type="text"
              value={customStyleInput}
              onChange={(e) => setCustomStyleInput(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter' && customStyleInput.trim()) setCollapsed(true);
              }}
              onBlur={() => {
                if (customStyleInput.trim()) setCollapsed(true);
              }}
              placeholder="예: 미야자키 하야오 풍"
